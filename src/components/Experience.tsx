...
const hackathons: HackathonItem[] = [
  {
    ...
    backImage: "https://i.postimg.cc/26pKLmDS/Screenshot-2026-03-27-222436.png",
  ...
  },
  {
    ...
    backImage: "https://i.postimg.cc/SNyd4Q0H/image.png",
  ...
  },
  {
    ...
    backImage: "https://images.unsplash.com/photo-1517245386807-bb43f82c33c4?auto=format&fit=crop&q=80&w=800",
    link: "#"
  },
  {
    ...
    backImage: "https://images.unsplash.com/photo-1620712943543-bcc4688e7485?auto=format&fit=crop&q=80&w=800",
    link: "#"
  }
];
...
const Experience = () => {
  ...
  useGSAP(() => {
    ...
    // Kill any existing ScrollTriggers for this section to prevent stale references
    ScrollTrigger.getAll().forEach(st => {
      if (st.trigger === sectionRef.current) st.kill();
    });

    // Pin the entire section and create scrub timeline
    const tl = gsap.timeline({
      scrollTrigger: {
        ...
        onUpdate: (self) => {
          ...
          const idx = Math.min(
            Math.floor(progress * hackathons.length),
            hackathons.length - 1
          );
          setActiveIndex(idx);
        }
    ...
    // Initial state: first item visible, rest hidden with pointer-events managed
    ...
    // Create sequence: old exits first, then new enters (slight overlap for smoothness)
    hackathons.forEach((_, i) => {
      if (i > 0) {
        // Exit old item
        tl.to(`.hack-text-${i - 1}`, { autoAlpha: 0, y: -40, pointerEvents: 'none', duration: 0.5, ease: 'power2.in' }, `step${i}`)
          ...
          // Spacer to give breathing room before next transition
          .to({}, { duration: 0.3 });
      }
    });
  ...
  return (
    <section id="experience" className="bg-[#050505] relative w-full h-screen overflow-hidden" ref={sectionRef}>
      <MatrixRain />
      {/* Background Glow Elements */}
      <div className="absolute top-1/4 right-0 w-[400px] h-[400px] md:w-[500px] md:h-[500px] bg-primary/5 rounded-full blur-[120px] pointer-events-none" />
      <div className="absolute bottom-0 left-0 w-[500px] h-[500px] md:w-[600px] md:h-[600px] bg-primary/5 rounded-full blur-[150px] pointer-events-none" />

      <div className="container mx-auto px-4 sm:px-6 h-full flex flex-col justify-center relative z-10 py-12 sm:py-16 lg:py-24 pb-8 sm:pb-12" ref={containerRef}>

        {/* Top Header with Counter */}
        <div className="relative mb-4 sm:mb-6 lg:mb-10 z-20 flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
          <div>
            <p className="reveal-title text-primary font-mono text-xs sm:text-sm tracking-widest uppercase mb-2 sm:mb-3 text-shadow-[0_0_10px_rgba(34,211,238,0.3)] invisible">Experience</p>
            <h2 className="reveal-title text-3xl sm:text-4xl md:text-5xl font-bold text-foreground tracking-tight mb-2 sm:mb-4 leading-tight invisible">Hackathon Journey</h2>
            <p className="reveal-title text-muted-foreground text-xs sm:text-sm lg:text-base leading-relaxed max-w-xl invisible">
              Battle-tested under pressure in competitive hackathons. Scroll to explore my projects and achievements.
            </p>
          </div>

          {/* Hackathon Counter */}
          <div className="reveal-title flex items-center gap-3 invisible shrink-0">
            <div className="flex items-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 rounded-xl bg-white/5 border border-white/10 backdrop-blur-sm">
        ...
        {/* Step Progress Bar */}
        <div className="reveal-title relative mb-4 sm:mb-6 lg:mb-8 z-20 invisible">
          <div className="flex gap-1.5 sm:gap-2">
            {hackathons.map((_, i) => (
              <div
                key={`bar-${i}`}
                className={`h-1 rounded-full flex-1 transition-all duration-500 ease-out ${
                  i <= activeIndex
        ...
        {/* Content Area */}
        <div className="flex flex-col lg:flex-row h-full w-full items-center flex-1 max-h-[55vh] sm:max-h-[60vh]">

          {/* Left Side: Overlapping Text Items */}
          <div className="w-full lg:w-1/2 relative h-full flex items-center pr-0 lg:pr-8 xl:pr-12 min-h-[50%] lg:min-h-0 z-30">
            {hackathons.map((hack, i) => (
              <div key={`text-${i}`} className={`hack-text-${i} absolute inset-0 flex flex-col justify-center space-y-3 sm:space-y-4 lg:space-y-5 w-full lg:w-[95%] invisible`}>
                <div>
                  <h3 className="text-xl sm:text-2xl lg:text-3xl xl:text-4xl font-bold text-white leading-tight mb-1.5 sm:mb-2 lg:mb-3">{hack.name}</h3>
                  <div className="h-0.5 sm:h-1 w-12 sm:w-16 bg-primary/60 rounded-full mb-3 sm:mb-4 lg:mb-5 shadow-[0_0_15px_rgba(34,211,238,0.5)]" />
                </div>

                <div className="flex flex-wrap gap-2 sm:gap-3 items-center">
                  <span className="flex items-center gap-1.5 sm:gap-2 text-[10px] sm:text-xs lg:text-sm font-mono px-2.5 py-1 sm:px-3 sm:py-1.5 lg:px-4 lg:py-2 rounded-lg sm:rounded-xl bg-white/5 text-white/80 border border-white/10 backdrop-blur-sm">
                    <Building2 className="w-3 h-3 sm:w-3.5 sm:h-3.5 lg:w-4 lg:h-4 text-primary" />
                    {hack.organization}
                  </span>
                  <span className="flex items-center gap-1.5 sm:gap-2 text-[10px] sm:text-xs lg:text-sm font-mono px-2.5 py-1 sm:px-3 sm:py-1.5 lg:px-4 lg:py-2 rounded-lg sm:rounded-xl bg-primary/10 text-primary border border-primary/20 backdrop-blur-sm">
                    <Trophy className="w-3 h-3 sm:w-3.5 sm:h-3.5 lg:w-4 lg:h-4" />
                    {hack.achievement}
                  </span>
                </div>
                ...
                <div className="flex flex-wrap gap-1.5 sm:gap-2">
                  {hack.techStack.map((tech, ti) => (
                    <span key={ti} className="flex items-center gap-1 text-[9px] sm:text-[10px] lg:text-xs font-mono px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-md bg-primary/5 text-primary/70 border border-primary/10">
                ...
                <div className="text-white/60 text-xs sm:text-sm lg:text-base pt-1 sm:pt-2">
                  {hack.description}
                </div>
                ...
                {hack.link !== "#" && (
                  <div className="pt-1 sm:pt-2">
                    ...
                    >
                      Explore Project <ExternalLink className="w-3.5 h-3.5 sm:w-4 sm:h-4 ml-1" />
                    </a>
                  </div>
                )}
              </div>
          ...
          {/* Right Side: Image Cards — no flip for disableFlip items */}
          <div className="w-full lg:w-1/2 relative h-[200px] sm:h-[250px] lg:h-[85%] mt-4 sm:mt-6 lg:mt-0 perspective-1000 z-20 flex items-center justify-center pointer-events-none">
            {hackathons.map((hack, i) => (
              <div
                key={`img-${i}`}
                className={`hack-img-${i} absolute inset-0 w-full h-full lg:h-[85%] my-auto group [perspective:1000px] invisible`}
              >

                <div className={`relative w-full h-full transition-transform duration-700 [transform-style:preserve-3d] ${hack.disableFlip ? '' : 'group-hover:[transform:rotateY(180deg)]'} shadow-2xl rounded-lg sm:rounded-xl lg:rounded-2xl cursor-default`}>

                  {/* Front Side */}
                  <div className={`absolute inset-0 w-full h-full ${hack.disableFlip ? '' : '[backface-visibility:hidden]'} rounded-lg sm:rounded-xl lg:rounded-2xl overflow-hidden border border-white/10 bg-[#0a0a0a]`}>
                    ...
                    <img
                      ...
                      loading="lazy"
                    />
                    <div className="absolute inset-0 ring-1 ring-inset ring-white/10 rounded-lg sm:rounded-xl lg:rounded-2xl z-20 pointer-events-none" />
                  </div>

                  {/* Back Side — only rendered for flip-enabled cards */}
                  {!hack.disableFlip && (
                    <div className="absolute inset-0 w-full h-full [backface-visibility:hidden] [transform:rotateY(180deg)] rounded-lg sm:rounded-xl lg:rounded-2xl overflow-hidden border border-primary/30 bg-[#0a0a0a]">
                      <img
                        src={hack.backImage}
                        alt={`${hack.name} Back`}
                        ...
                        loading="lazy"
                      />
                      <div className="absolute inset-0 ring-1 ring-inset ring-primary/20 rounded-lg sm:rounded-xl lg:rounded-2xl z-20 pointer-events-none" />
                    </div>
                  )}
